...
To make these changes fast you wrote a class to translate a camelCase name into an underscore separated name. Implement the ToUnderscore() method.
...
const toUnderScore = name => {
  const upperC = 'ABCDEFGHIJKLMNOPQRSTUVWXQYZ'
  const numbers = '1234567890'
  return name
    .split('')
    .map((el, i, arr) => {
      if (
        (i !== 0 && upperC.includes(el) && arr[i-1] !== '_') ||
        (i !== 0 && numbers.includes(el) && !numbers.includes(arr[i-1]) && arr[i-1] !== '_')
      ) {
        return '_' + el
      } else {
        return el
