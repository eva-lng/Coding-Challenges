...
function getWinner(listOfBallots) {
  const votes = {};
  const majorityNum = listOfBallots.length / 2;
  listOfBallots.forEach(name => votes[name] = (votes[name] || 0) + 1);
  const winner = Object.keys(votes).filter(el => {if (votes[el] > majorityNum) return el});
